...
CREATE OR REPLACE VIEW {{env}}_alecs_bytes.harmonized.daily_weather_v
...
JOIN {{env}}_alecs_bytes.raw_pos.country c
    ON c.iso_country = hd.country
    AND c.city = hd.city_name;

-- Apply UDFs and confirm successful execution
CREATE OR REPLACE VIEW {{env}}_alecs_bytes.harmonized.weather_hamburg
AS
SELECT
...
CREATE OR REPLACE VIEW {{env}}_alecs_bytes.analytics.daily_city_metrics_v
...
SELECT
    ...
    ZEROIFNULL(SUM(odv.price)) AS daily_sales,
    ROUND(AVG(fd.avg_temperature_air_2m_f),2) AS avg_temperature_fahrenheit,
    ROUND(AVG({{env}}_alecs_bytes.analytics.fahrenheit_to_celsius(fd.avg_temperature_air_2m_f)),2) AS avg_temperature_celsius,
    ROUND(AVG(fd.tot_precipitation_in),2) AS avg_precipitation_inches,
    ROUND(AVG({{env}}_alecs_bytes.analytics.inch_to_millimeter(fd.tot_precipitation_in)),2) AS avg_precipitation_millimeters,
    MAX(fd.max_wind_speed_100m_mph) AS max_wind_speed_100m_mph
FROM {{env}}_alecs_bytes.harmonized.daily_weather_v fd
LEFT JOIN {{env}}_alecs_bytes.harmonized.orders_v odv
    ON fd.date_valid_std = DATE(odv.order_ts)
    AND fd.city_name = odv.primary_city
    AND fd.country_desc = odv.country
GROUP BY fd.date_valid_std, fd.city_name, fd.country_desc;
...
CREATE OR REPLACE VIEW {{env}}_alecs_bytes.harmonized.windspeed_hamburg
    AS
SELECT
    dw.country_desc,
    dw.city_name,
